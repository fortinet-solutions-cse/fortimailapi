...
if __name__== "__main__":
    ...
    fml.debug("on")
    fml.https("off")
    fml.login("192.168.122.12","admin","")

    log.debug(fml.license())

    log.debug(fml.get_version())
    if Version(fml.get_version()) > Version('5.3'):
        log.debug("Version is later than 5.3")
    else:
        log.debug("Version is older than 5.3")
